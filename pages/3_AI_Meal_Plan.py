...
def get_detailed_recipes(meal_plan, preferences, consciousness_level, meal_agent):
    """Generate detailed recipes for the meal plan"""
    
    # Extract meal names from the plan
    meal_requests = []
    if "weekly_meals" in meal_plan:
        for meal_type, meals in meal_plan["weekly_meals"].items():
            for meal in meals[:2]:  # Get recipes for top 2 options per meal type
                meal_requests.append({
                    "type": meal_type,
                    "name": meal.get("name", f"{meal_type} option"),
                    "prep_time": meal.get("prep_time", "moderate"),
    ...
    # Create recipe request prompt
    recipe_prompt = f"""
SOVEREIGNTY RECIPE REQUEST

User Context:
- Consciousness Level: {consciousness_level}
- Diet Type: {preferences.get('diet_type', 'Omnivore')}
- Cooking Time: {preferences.get('cooking_time', 'Moderate')}
- Dietary Restrictions: {preferences.get('restrictions', [])}
- Budget Level: {preferences.get('budget_level', 'Moderate')}
- Calorie Tier: {preferences.get('calorie_tier', 'Moderate')}

Meals to Create Recipes For:
{chr(10).join([f"- {meal['type'].title()}: {meal['name']} (Prep: {meal['prep_time']})" for meal in meal_requests])}

Please provide detailed, sovereignty-aligned recipes for each meal that match the user's {consciousness_level} consciousness level.

For each recipe, include:
1. Ingredients list with realistic quantities and costs
2. Step-by-step cooking instructions
3. Prep time, cook time, and total time
4. Nutritional highlights (macro breakdown if Orange level, holistic benefits if Green/Teal/Turquoise)
5. Sovereignty benefits and why this recipe builds independence
6. Storage and meal prep instructions
7. Variations or substitutions

Adjust language complexity and philosophical depth to match their {consciousness_level} level:
- Orange: Focus on precise measurements, timing, nutritional data, efficiency
- Green: Emphasize ingredient sourcing, mindful preparation, seasonal awareness
- Teal: Include systems thinking about food preparation, adaptive techniques
- Turquoise: Integrate spiritual aspects of cooking, consciousness through food preparation

Format as a comprehensive recipe collection that empowers sovereignty through cooking mastery.
"""

    try:
        # Create new thread for recipes
        thread = meal_agent.client.beta.threads.create()
        
        meal_agent.client.beta.threads.messages.create(
            thread_id=thread.id,
            role="user",
            content=recipe_prompt
        )
        
        run = meal_agent.client.beta.threads.runs.create(
            thread_id=thread.id,
            assistant_id=meal_agent.assistant_id
        )
        
        # Wait for completion
        while True:
            run = meal_agent.client.beta.threads.runs.retrieve(thread_id=thread.id, run_id=run.id)
            if run.status == "completed":
                break
            elif run.status in ["failed", "cancelled", "expired"]:
                return None
        ...
        # Get response
        messages = meal_agent.client.beta.threads.messages.list(thread_id=thread.id)
        recipes_response = messages.data[0].content[0].text.value
...
def display_detailed_recipes(recipes_text):
    ...
    st.markdown("---")
    st.markdown("## 👨‍🍳 Detailed Sovereignty Recipes")
    st.markdown("*Master your kitchen, master your freedom*")
    
    # Create expandable recipe sections
    with st.expander("📖 Complete Recipe Collection", expanded=True):
        # Style the recipes with better formatting
        st.markdown(f"""
        <div style="background: rgba(16, 185, 129, 0.05); padding: 20px; border-radius: 10px; border-left: 4px solid #10b981;">
        {recipes_text.replace(chr(10), '<br>')}
        </div>
        """, unsafe_allow_html=True)
    
    # Recipe download button
    def create_recipe_download():
        # Create a simple text file with recipes
        recipe_content = f"""SOVEREIGNTY RECIPES
Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}

{recipes_text}

---
Generated by Sovereignty Score AI Meal Planning
Building independence through conscious nutrition
"""
        return recipe_content.encode('utf-8')
    
    st.download_button(
        label="📥 Download Complete Recipe Collection",
        data=create_recipe_download(),
        file_name=f"sovereignty_recipes_{datetime.now().strftime('%Y%m%d')}.txt",
        mime="text/plain",
